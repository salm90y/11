...
def dictionary_lookup_word_correction(word: str) -> str:
    ...
    if not clean_w or len(clean_w) < 3 or clean_w.isdigit():
        return word

    # تصحيح الهاء إلى تاء مربوطة للأسماء المؤنثة
    ...
    # لا نستخدم difflib العشوائي لمنع استبدال الأسماء والمواد القانونية بكلمات غريبة
    return word
...
def normalize_arabic_word_text(text: str) -> str:
    ...
    legal_rules = [
        ...
        (r'وكالة الوزارة لشؤون الاسم|وكالة الوزارة لشؤون الأمن الاتحال', 'وكالة الوزارة لشؤون الشرطة'),
        (r'مديرية شرطسة|مذيري شرطسة|مديريةشرطة', 'مديرية شرطة'),
        ...
        (r'لقسوى الاسم الداخليـة|لقسوى الأمن الداخلي|قوى الاسم الداخليـة', 'لقوى الأمن الداخلي'),
        (r'من قسانون|قسانون أصول', 'من قانون أصول'),
        (r'قتانون عقوبات', 'قانون عقوبات'),
        (r'تنادأ لأخك ام|استنادأ لأحكام|تنادأ لأحكام', 'واستناداً لأحكام'),
        (r'استنادأ|استنادا', 'واستناداً'),
        (r'وأللف الم ادتين|وأللف المادتين|والف المادتين', 'و(44 خامساً)'),
        (r'الم ادتين|المادتين', 'المادتين'),
        (r'ثالثأ|ثالثا', 'ثالثاً'),
        (r'خامسأ|خامسا', 'خامساً'),
        (r'أولأ|أولا', 'أولاً'),
        (r'بناءا ى الم|بناءا على|وبناءا', 'وبناءً على'),
        (r'امر لاحيات|امر الصلاحيات|الصلاحيات المخولة الينا', 'وفقاً للصلاحيات المخولة إلينا'),
        (r'وفقأ', 'وفقاً'),
        (r'لوائذ العقوبة|لوائح العقوبة', 'العقوبة المبينة'),
        (r'إزاءكلوحدمنهم|إزاء كل واحدمنهم', 'إزاء كل واحد منهم'),
        (r'قسائمة اسماء|قائمة اسماء', 'قائمة أسماء'),
        (r'صورة عنه ال|صورة منه ال', 'صورة منه إلى: '),
        (r'الشعبة القانونية لواا|الشعبة القانونية', 'الشعبة القانونية'),
