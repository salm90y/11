...
export default function DocumentList({ documents, onDelete, onView }: DocumentListProps) {
  ...
  return (
    <div className="space-y-6" dir="rtl">
      {/* تبويب نمط العرض الفخم */}
      <div className="bg-slate-100 p-1.5 rounded-2xl flex border border-slate-200 text-sm font-bold w-full md:w-fit" id="view-mode-tabs">
        <button
          id="folders-view-btn"
          onClick={() => setViewMode('folders')}
          className={`flex-1 md:flex-initial px-6 py-2.5 rounded-xl transition-all flex items-center justify-center gap-2 ${
            viewMode === 'folders' 
        ...
        >
          <FolderOpen size={18} />
          <span>📂 تصفح المجلدات والأرشيف الهيكلي</span>
        </button>
        <button
          id="grid-view-btn"
        ...
        >
          <LayoutGrid size={18} />
          <span>🔍 عرض شبكة المستندات</span>
        </button>
      </div>

      {viewMode === 'folders' ? (
        <FolderExplorer documents={documents} onView={onView} onDelete={onDelete} />
      ) : (
                ...
                <div className="p-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-400 font-mono">
                      <FileText size={14} />
                      رقم المستند: {doc.letterNumber || doc.id}
                    </div>
                  </div>
