...
LOCAL_ARABIC_DICTIONARY = set([
    "وزارة", "الداخلية", "الدفاع", "المالية", "العدل", "الصحة", "التربية", "التعليم",
    "وكالة", "الوزارة", "لشؤون", "الأمن", "الاتحادي", "الوطني", "المرور", "الشرطة",
    "مديرية", "قسم", "شعبة", "وحدة", "إدارة", "الإدارة", "المالية", "الموارد", "البشرية",
    "الطاقة", "الكهرباء", "النفط", "التقاعد", "العامة", "القيادة", "اللواء", "الكتيبة",
    "التاريخ", "العدد", "الموضوع", "إلى", "من", "السيد", "المحترم", "المحترمون",
    "تدرجكم", "أدناه", "أعلامنا", "انفكاككم", "مباشرتكم", "انفكاك", "مباشرة",
    "يرجى", "التفضل", "بالاطلاع", "واتخاذ", "ما", "يلزم", "الإجراءات", "الأصولية",
    "وبناءً", "على", "كتاب", "إشارة", "أمر", "إداري", "شخصي", "خدمة", "ملاك",
    "ملاحظة", "الهوية", "الرقم", "الوطني", "الموافق", "السنة", "الشهر", "اليوم",
    "الاسم", "الثلاثي", "اللقب", "الرتبة", "العسكرية", "الراتب", "عظيم", "قدوتنا",
    "خلق", "القرآن", "شكر", "وتقدير", "المعني", "المذكور", "أعلاه"
])

# قائمة التعبيرات والجمل الرسمية الثابتة
OFFICIAL_PHRASES_MAP = {
...
def dictionary_lookup_word_correction(word: str) -> str:
    """
    مطابقة الكلمة مع معجم الكلمات العربية المحلية باستخدام خوارزميات الشبه الضوئي
    لتصحيح الأحرف التشابهية (د/ذ، ر/ز، ه/ة، س/ش، ع/غ، ط/ظ، ح/خ/ج)
    """
    if not word or len(word) < 3:
        return word

    import re
    # تنظيف الحركات والعلامات عن الكلمة المفردة
    clean_w = re.sub(r'[^\w\s]', '', word)
    if not clean_w or clean_w.isdigit():
        return word

    # إذا كانت الكلمة موجودة مباشرة في المعجم
    if clean_w in LOCAL_ARABIC_DICTIONARY:
        return word

    # استخدام difflib للبحث عن الكلمة الأقرب في المعجم العربي بنسبة تطابق عالية (>= 78%)
    matches = difflib.get_close_matches(clean_w, LOCAL_ARABIC_DICTIONARY, n=1, cutoff=0.78)
    if matches:
        corrected_base = matches[0]
        return word.replace(clean_w, corrected_base)
...
def normalize_arabic_word_text(text: str) -> str:
    ...
    2. استبدال التراكيب الحكومية والرسمية من القاموس
    3. معالجة أخطاء اشتباه مد الحروف بحرف "سين" (س/سس)
    4. تطبيق Dictionary Lookups (القاموس الذكي) على مستوى الكلمات لتصحيح الأحرف التشابهية
    """
    if not text:
        return ""
    ...
    # 2. استبدال العبارات والتراكيب الرسمية الثابتة أولاً
    cleaned = text
    for phrase, correction in OFFICIAL_PHRASES_MAP.items():
        cleaned = re.sub(re.escape(phrase), correction, cleaned, flags=re.IGNORECASE)

    # 3. تصحيح اشتباه مد الحروف بحرف السين المكرر (مثل سس/سسس/سـ) الناتجة عن وصلات السكانر
    word_replacements = [
    ...
    # 4. معالجة Dictionary Lookups لكل كلمة على حدة لتصحيح تشابه الأحرف
    processed_lines = []
    for line in cleaned.split('\n'):
        ...
        corrected_words = [dictionary_lookup_word_correction(w) for w in words]
